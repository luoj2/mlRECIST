...
out = open('../log/predict_objective/log_' + str(FLAGS.learning_rate) + "_" + str(FLAGS.init) + "_" + str(FLAGS.batch_size)+ "_" + str(FLAGS.num_epochs) + ".txt",'w')
out.write("\nParameters:\n")
print("\nParameters:")
...
#             tf.set_random_seed(1)
            model = Model(4,len(word_to_id),FLAGS.hidden_dim,FLAGS.learning_rate,FLAGS.init)
    ...
    #                     print("Ground truth",y_test)
                        ...
                        with open('../log/predict_objective/split_' + str(split) + ".txt",'w') as rec:
                            for i in range(len(pre)):
